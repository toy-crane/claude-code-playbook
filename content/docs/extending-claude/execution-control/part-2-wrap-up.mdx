...
- **MCP**: CLI 가 대세가 된 지금도 MCP 가 더 나은 세 가지 상황이 있습니다 — 실행 중인 앱 관찰 (Chrome DevTools · Playwright · Figma), OAuth 가 복잡한 외부 서비스 (Linear · Notion · Sentry), `.mcp.json` 을 공유하는 팀 설정. Claude Code (Host + Client) 가 서버 프로세스를 띄워 stdio 로 통신합니다
