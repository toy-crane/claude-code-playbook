...
description: Claude in Chrome 으로 내 Chrome 세션에 접근해 Claude 가 쓸 수 있는 Tool 을 확장합니다
public: true
---
...
앞 레슨에서 gh CLI 로 Claude 가 실시간 GitHub 데이터를 가져오게 만들었습니다. 그래도 CLI 만으로는 채워지지 않는 영역이 있었습니다 — 지금 열어둔 크롬 탭 상태, 챗 앱의 인터랙티브 UI.

이번 레슨은 이런 영역을 **MCP** 로 해결합니다. Claude Code 에 이미 들어 있는 **Claude in Chrome** 으로 내 실제 브라우저 세션에 접근해, Claude 가 쓸 수 있는 Tool 이 한눈에 늘어나는 것을 체험합니다.
...
<Callout type="info" title="설치 후 로그인을 한 번 해야 합니다">
...
1. **MCP 를 붙일 때마다 Claude 의 Tool 이 늘어납니다**: `--chrome` 으로 Claude in Chrome 을 켜면 내 실제 Chrome 세션을 다루는 Tool 이 추가됩니다. 외부 MCP 도 `claude mcp add` 로 같은 방식으로 붙입니다
...
- **Q: 팀원과 `.mcp.json` 으로 공유 가능한가요?**
  - A: 외부 MCP 를 `claude mcp add` 시 `-s project` 로 추가하면 `.mcp.json` 에 저장돼 팀원과 같은 설정을 공유합니다. Claude in Chrome 은 `--chrome` 플래그 방식이라 `.mcp.json` 공유에 대한 공식 가이드가 없으니, 각자 설치하는 쪽이 안전합니다
