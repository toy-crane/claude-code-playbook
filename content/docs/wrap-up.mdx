---
title: 전체 코스 돌아보기
description: 'Claude Code 전체 코스를 간결하게 요약하고, Compound Engineering과 병렬 작업 등 이후 학습 방향을 안내합니다'
public: true
---
...
- **SDD**: 엔지니어링 템플릿의 Skills(/write-spec, /sketch-wireframe, /draft-plan, /execute-plan, /self-improve)와 Hooks로 Ideate → Specify → Sketch → Plan → Build → Compound 사이클을 구조화. feedme.wiki 클론 예제로 전체 사이클을 경험하고 Vercel에 배포
...
[Chapter 10 Lesson 01](./completing-projects/parallel-work/git-worktree-isolation)에서 git worktree와 `claude --worktree`의 기본 워크플로우를 배웠습니다. 여기서 더 나아갈 수 있는 방향입니다.
