---
title: 사전 준비사항
description: Claude Code for Developers 강의 수강 전 설치해야 할 도구와 계정을 안내합니다
public: true
---
...
1. 회사 이메일 계정에 접속하여 Anthropic이 보낸 팀 플랜 초대 메일을 확인합니다

![prerequisites-team-invite](/learn/prerequisites-team-invite.png)

2. 해당 이메일의 **Accept invite** 링크를 사외에서 열어 Claude 회원가입 및 Team 등록을 완료합니다

<Callout type="warn" title="초대 메일을 수신한 LGE 도메인으로만 등록 가능합니다." />
...
설치가 완료되면 **VS Code 터미널을 닫았다가 다시 열어야** `claude` 명령어를 인식합니다.
...
<Callout type="info" title="버전 번호는 다를 수 있습니다">
위 숫자와 정확히 같을 필요는 없습니다. 숫자가 표시되면 설치가 완료된 것입니다.
</Callout>

<Callout type="warn" title="Windows에서 `claude` 명령어를 찾을 수 없는 경우">

`claude --version` 실행 시 다음과 비슷한 오류가 표시되면 PATH가 아직 갱신되지 않은 상태입니다.
...
1. **VS Code 터미널을 닫았다가 다시 엽니다.** 새 터미널에서 PATH가 갱신됩니다.
2. 터미널이 **PowerShell**(또는 pwsh)인지 다시 확인합니다. cmd 셸이면 터미널 우측 상단의 `+` 옆 드롭다운에서 PowerShell을 선택합니다.
3. 그래도 인식되지 않으면 설치 명령을 PowerShell에서 다시 실행한 뒤 VS Code 터미널을 닫고 다시 엽니다.
...
<Callout type="idea" title="VS Code 확장 프로그램">
...
Node.js는 JavaScript 런타임이며, 설치하면 패키지 매니저인 **npm**과 패키지 실행 도구인 **npx**가 함께 설치됩니다. 강의에서 다루는 MCP 서버 설치와 일부 보조 도구가 `npx`를 통해 실행되므로 반드시 설치해 주세요.

<Callout type="warn" title="Node.js를 설치하지 않으면 자주 만나는 오류">
...
<Callout type="warn" title="Windows에서 설치가 안 되는 경우">
